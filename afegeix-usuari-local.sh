...
# 1-Asegurar que el script lo ejecuta el superusuario (root).
if [[ "${UID}" -eq 0 ]]
then
# 2-Pide el nombre del usuario (login).
...
# 8-Comprueba si la contraseña se le ha otorgado de manera correcta.
        if [[ $? -eq 0 ]]
        then
                echo "Comprobación de que la contraseña se le ha dado correctamente"
        else
                echo "ha ocurrido un error"
                exit 1
        fi
...
# 10-Muestra el usuario, la contraseña y desde que host se ha creado.
        ...
        echo "No puedes ejecutar este script, inténtalo más tarde con el usuario root"
fi
